...
Tp* anycast_impl(Any& any) {
    using Decayed = Any::ValueType<Tp>;
    if (any.current_object_type_ == nullptr || typeid(Decayed) != *any.current_object_type_) {
        throw std::runtime_error("Bad any cast");
    }
    return static_cast<Tp*>(Any::Manager<Tp>::access_object(any.storage_));
}


template <typename T>
T anycast(Any& any) {
    auto* result = anycast_impl<T>(any);
    return *result;
}
...
int main() {
    ...
    any2 = std::move(any);
    // any2.emplace<std::string>("fucking nigger");
